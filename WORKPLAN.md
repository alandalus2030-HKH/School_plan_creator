...
### اليوم 51-52 | نظام Onboarding للمدارس الجديدة ✅ 2026-06-08
- [x] `src/app/onboarding/page.tsx` — معالج 3 خطوات مستقل (حارس super-admin):
  - الخطوة 1: المدرسة + الشعار (معاينة) + بيانات الاتصال
  - الخطوة 2: حساب المدير الأول (اسم/بريد/دخول/كلمة مرور)
  - الخطوة 3: خطة أولى **اختيارية** (اسم/عام/عدد مستويات) — قابلة للتخطّي
  - شاشة نجاح + "الدخول إلى المدرسة" (تقمّص) أو العودة لإدارة المدارس
- [x] `src/app/api/onboarding/route.ts` — نقطة موحّدة (multipart) لمشرف النظام:
  مدرسة + اتصال + شعار (admin storage) + مدير + خطة، مع تراجع عند الفشل
- [x] زر "معالج الإعداد" في صفحة إدارة المدارس
- [x] طُبّق درس insertBefore (عزل الأيقونات الشرطية في span)
- [x] Build نظيف ✓
...
### اليوم 56-57 (ب) | الصورة الشخصية ✅ 2026-06-09
- [x] `api/profile/avatar` (رفع/حذف عبر service role، bucket: avatars)
- [x] عرض الصورة في الملف الشخصي + الشريط الجانبي (userAvatar في السياق)

### اليوم 58 | ~~عبارات تحفيزية~~ → منصة التتويج (موظف الشهر) ✅ 2026-06-09
> قرار: استُبدلت العبارات التحفيزية (أثر ضعيف) بـ **منصة تتويج** أعلى أثراً تستثمر نظام الأوسمة/النقاط + الصورة الشخصية.
- [x] `migrations/020` — schools.featured_employee_id + featured_note
- [x] `api/featured-employee` (GET/POST/DELETE — manage_settings)
- [x] `RecognitionPodium.tsx` — منصة أفضل 3 (هجين: تلقائي بالنقاط هذا الشهر + تثبيت يدوي للمدير)
- [x] عرضها في لوحة التحكم + "مهامي" (ليراها الموظفون والمدراء)

### اليوم 59-60 | الاختبار الشامل + الإطلاق → 🔀 رُحّلا إلى `WORKPLAN_V2.md`
> **قرار 2026-06-09:** بما أننا أضفنا مبادرة كبرى (سير عمل المهام والأدلة)،
> رُحّل الاختبار النهائي والإطلاق لنهاية الخطة الجديدة `WORKPLAN_V2.md`
> (الأسبوع الثالث، اليومان 11-12) — لنختبر ونُطلق النظام مكتملاً لا ناقصاً.
